...
* */
class HomePaginatedThirdSectionTabDataAdapter constructor(val context: Activity) :
    ...
    var loadMore: MutableLiveData<Int> = MutableLiveData<Int>()
    var pageCounter = 1
    var currentSize = 0
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return if (viewType == TYPE_FOOTER) {
            FooterViewHolder(
                DataBindingUtil.inflate<LayoutProgressVerticalBinding>(
                    LayoutInflater.from(context),
                    R.layout.layout_progress_vertical, parent, false
                )
            )
        } else {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is FooterViewHolder) {
          ...
          //  holder.binding.tvSecondAuthor.paintFlags = holder.binding.tvSecondAuthor.paintFlags or Paint.UNDERLINE_TEXT_FLAG
    ...
    fun provideData(dataList: List<Featurednews>) {
        ...
        pageCounter=1
        pageCounter=1
        var a = ArrayList<Featurednews>(dataList)
        this.dataList = a
    ...
    inner class FooterViewHolder constructor(internal var binding: LayoutProgressVerticalBinding) :
        RecyclerView.ViewHolder(binding.root)
}